...
class reminder extends Home
{
    ...
    */
    public function reminders_data()
    {
        ...
        $join = array(
            'member' => 'circulation.member_id = member.id,left',
            'book_info' => 'circulation.book_id = book_info.id,left'
            );
        $select = array('circulation.id as id', 'member.name as name', 'member.user_type', 'member.id as member_id', 'member.email as email', 'member.mobile', 'book_info.title as title', 'issue_date', 'expire_date');
